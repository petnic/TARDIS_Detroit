...
void setup() {
  ...
  int valueA1 = analogRead(A1); // A
  previousValueA1 = valueA1;
}

int loopDelay = 100; // 100 milliseconds
bool play = false;
...
void loop() {
  ...
  int valueA0 = digitalRead(A0); // OUT
  int valueA1 = analogRead(A1); // A
  int valueA2 = digitalRead(A2);
  ...
  int difference = abs(valueA1 - previousValueA1);
  ...
  if (valueA2 == HIGH)
  {
    if (difference > 25)
    {
      ...
      {
        Serial.println("startPlayingFile");
